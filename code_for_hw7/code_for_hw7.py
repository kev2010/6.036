...
class Tanh(Module):            # Layer activation
    ...
    def backward(self, dLdA):    # Uses stored self.A dLdA = (m,b), self.A = (n,b).
...
class SoftMax(Module):           # Output activation
    def forward(self, Z):
        temp = np.exp(Z)
        tot = np.diag(1/np.sum(temp, axis=0))
        return temp@tot    # Your code: (?, b)

    def backward(self, dLdZ):    # Assume that dLdZ is passed in
        return dLdZ

    def class_fun(self, Ypred):  # Return class indices
        return Ypred.argmax(axis=0)
...
class NLL(Module):       # Loss
    def forward(self, Ypred, Y):
        self.Ypred = Ypred     # (n,b)
        self.Y = Y             # (n,b)
        return -np.sum(Y*np.log(Ypred))       # (n)
...
class Sequential:
    ...
    def sgd(self, X, Y, iters=100, lrate=0.005):  # Train
        ...
        for it in range(iters):
            p = np.random.randint(0, high=(N-1))
            pred = self.forward(X[:,p:p+1])
            loss = self.backward(pred-Y[:,p:p+1])
            self.sgd_step(lrate)
